...
import 'package:taskova_shopkeeper/view/Specific_detailed/job_Postdetails.dart';
...
//   }
// }
// Add this class to your existing my_jobpost.dart file
import 'dart:convert';
import 'package:flutter/material.dart';
import 'package:flutter_dotenv/flutter_dotenv.dart';
import 'package:http/http.dart' as http;
...
class MyJobpostContent extends StatefulWidget {
  const MyJobpostContent({Key? key}) : super(key: key);

  @override
  State<MyJobpostContent> createState() => _MyJobpostContentState();
}

class _MyJobpostContentState extends State<MyJobpostContent> {
  final baseUrl = dotenv.env['BASE_URL'];
  String? _authToken;
  bool _isLoading = true;
  String? _errorMessage;
  ...
  // Safe setState - only call if widget is still mounted
  ...
  Future<void> fetchBusinesses() async {
    ...
    try {
      ...
      if (businessResponse.statusCode == 200) {
        ...
        // Fetch jobs for the selected business
        if (_businessData != null && _businessData!['id'] != null && mounted) {
          await fetchJobsForBusiness(_businessData!['id']);
        } else {
          _safeSetState(() {
            _isLoading = false;
  ...
  //Application count with improved error handling
  Future<int> fetchApplicantsCount(int jobId) async {
    if (!mounted) return 0;
    ...
    try {
      final request = http.Request(
        'GET',
        Uri.parse('$baseUrl/api/job-requests/list/$jobId/'),
      );
      request.headers.addAll(_getAuthHeaders());
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          ...
          print('Unexpected format: not a list for job $jobId');
        }
      } else if (response.statusCode == 404) {
        // Handle 404 gracefully - job might not have any applicants endpoint
        print('No applicants endpoint found for job $jobId (404)');
        return 0;
      } else {
        print(
          'Failed to fetch applicant count for job $jobId: ${response.statusCode}',
        );
      }
    } catch (e) {
  ...
  //Get posts of specific business
  Future<void> fetchJobsForBusiness(int businessId) async {
    if (!mounted) return;
    ...
    try {
      ...
      if (jobResponse.statusCode == 200) {
        ...
        // Create a list to store futures for applicant counts
        final List<Future<void>> applicantFutures = [];

        // Create futures for fetching applicant counts
        for (var job in jobData) {
          if (mounted) {
            applicantFutures.add(
              fetchApplicantsCount(job['id'])
                  .then((count) {
                    if (mounted) {
                      job['applicants_count'] = count;
                    }
                  ...
                  .catchError((error) {
                    print(
                      'Error fetching applicants for job ${job['id']}: $error',
                    );
                    if (mounted) {
        ...
        // Wait for all applicant count requests to complete
        if (mounted) {
          try {
            await Future.wait(applicantFutures);
          } catch (e) {
            print('Some applicant count requests failed: $e');
            // Continue anyway, some jobs will just show 0 applicants
          }
        }
  ...
  void _showPostJobOptions(BuildContext context) {
    showDialog(
      ...
      builder:
          (context) => AlertDialog(
            title: const Text("Choose Job Type"),
            content: const Text(
              "Do you want to hire for today or schedule for another day?",
            ),
            actions: [
              TextButton.icon(
                ...
                label: const Text("Hire for Today"),
                onPressed: () {
                  Navigator.of(context).pop(); // Close the dialog
                  Navigator.push(
                    context,
              ...
              TextButton.icon(
                ...
                label: const Text("Hire for Another Day"),
                onPressed: () {
                  Navigator.of(context).pop(); // Close the dialog
                  Navigator.push(
                    context,
            ...
            child: Row(
              ...
              children: [
                IconButton(
                  icon: const Icon(Icons.refresh),
                  tooltip: 'Refresh',
                  onPressed: _isLoading ? null : _loadTokenAndFetchData,
                ),
              ],
            ),
          ),
  ...
  Widget _buildBody() {
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [const SizedBox(height: 0), _buildJobsSection()],
        ),
      ),
    );
  }
